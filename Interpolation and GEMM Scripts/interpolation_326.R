...
sensors.c <- sensors %>% filter((provider == "clarity"|provider == "purple_air") & is_suspicious == FALSE)
...
data.v <- data.v %>% filter(!sensor_id %in% c(288, 79, 92,45)) #Drops outlier sensors
# Setup spatial objects
yerevan_boundary <- sf::st_read("C:/Users/mrealehatem/Documents/GitHub/ArmeniaPollutionAnalysis/Yerevan Boundary/boundary.gpkg",
